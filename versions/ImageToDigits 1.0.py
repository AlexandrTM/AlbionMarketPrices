...
def getDigitsFromImage(imagePath):
    ...
    #for i in range(1):
        for j in range(1):# if height needed
            imageRegion = image.crop((i * 6 - 1, 2, i * 6 + 7, 9))
            for k in range(4):
                imageSubRegion = imageRegion.crop((0 + k, 0, 5 + 3, 7))
                ...
                #print(math.floor(max(localDigitSimilarity) * 100) / 100, "digit" + str(localDigitSimilarity.index(max(localDigitSimilarity))))
                #digitSimilarity[k] = max(localDigitSimilarity)
                #digitSimilarity[localDigitSimilarity.index(max(localDigitSimilarity))] = digitSimilarity.pop(k)
            #print(math.floor(max(digitSimilarity.values())*100)/100, max(digitSimilarity, key=digitSimilarity.get))
            #print(digitSimilarity)
            #print(math.floor(max(digitSimilarityValues)*100)/100)
            print(digitSimilarityKeys[digitSimilarityValues.index(max(digitSimilarityValues))])
