...
def findDigitInImage(image, alphabet):
    digitSimilarityValues = []
    for i in range(len(alphabet)):
        digitSimilarityValues.append(getImageDifference(image, alphabet[i]))
        # print(str(digitSimilarityValues[i]) + " " + str(i))
    digit = str(digitSimilarityValues.index(max(digitSimilarityValues)) // 2)
    return digit
